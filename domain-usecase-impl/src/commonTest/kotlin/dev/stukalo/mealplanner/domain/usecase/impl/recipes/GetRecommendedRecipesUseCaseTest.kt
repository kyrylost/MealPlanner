...
class GetRecommendedRecipesUseCaseTest {
    @Test
    fun `given partially consumed day when getting recommendations then calculates adaptive target calories based on remaining pool`() {
        runTest {
            // Given
            val slots =
                listOf(
            ...
            // Mock dependencies
            val getMealScheduleUseCase =
                object : GetMealScheduleUseCase {
                    override fun invoke(): Flow<List<MealSlotDomainModel>> = flowOf(slots)
                }
            val getDailyNormUseCase =
                object : GetDailyNormUseCase {
                    override fun invoke(): Flow<DailyNormDomainModel?> = flowOf(norm)
                }
            val getDailyProgressUseCase =
                object : GetDailyProgressUseCase {
            ...
            val useCase =
                GetRecommendedRecipesUseCaseImpl(
    ...
    @Test
    fun `given no meals consumed when getting recommendations for first slot then calculates target based on total norm`() {
        runTest {
            // Given
            val slots =
                listOf(
