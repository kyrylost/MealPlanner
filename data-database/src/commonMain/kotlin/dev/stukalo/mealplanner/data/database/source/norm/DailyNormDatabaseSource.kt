...
class DailyNormDatabaseSource(private val dao: DailyNormDao) {
    suspend fun insert(dailyNorm: DailyNormDatabaseModel): Result<Unit> = runCatching {
        dao.insert(dailyNorm)
    }

    suspend fun getDailyNorm(): DailyNormDatabaseModel? = dao.getDailyNorm()

    fun getDailyNormAsFlow(): kotlinx.coroutines.flow.Flow<DailyNormDatabaseModel?> = dao.getDailyNormAsFlow()
}
